...
document.getElementById("noteBtnSave").onclick = function() {codenote.saveNote()};
...
let codenote = {
    ...
    mainNote: function () {
        ...
        if (sessionStorage.create === "create") {
            codenote.displayNote();
        }
        else if (typeof(sessionStorage.create) !== "undefined") {
            let note = codenote.getNoteByID(sessionStorage.create);
            codenote.displayNote(note);
        }
    ...
    saveNote: function () {
        ...
        if (id === "0") {
            ...
            model.addNoteToStorage(note.getNote());
        }
        else {
            // notiz editieren
            note = new Notiz(id, title, description, importance, date, finished, created);
            model.editNoteInStorage(note.getNote());
        }
    ...
    getNoteByID: function (id) {
        let allNotes = model.getSavedNotes();
